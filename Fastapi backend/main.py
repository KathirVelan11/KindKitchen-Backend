...
# Make stdout/stderr UTF-8 so emoji log lines don't crash on Windows consoles (cp1252)
try:
...
while True:
    try:
        conn = psycopg2.connect(
            database=os.getenv("DB_NAME", "kathir"),
            user=os.getenv("DB_USER", "postgres"),
            password=os.getenv("DB_PASSWORD", "12122005"),
...
CREATE TABLE IF NOT EXISTS Payments (
    ...
    PaymentMethod VARCHAR(50) NOT NULL CHECK (PaymentMethod IN ('Credit Card', 'Debit Card', 'UPI', 'Cash', 'Bank Transfer', 'Wallet')),
